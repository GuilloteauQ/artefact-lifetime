...
%\title{State-of-the-practice of artifacts lifetime in system conferences}
\title{Lifetime of artifacts in top system conferences: a~State-of-the-practice~review}
...
%\title{A lifetime-focused review of the State-of-the-practice for sharing artifacts in top system conferences}
...
We claim that the current state-of-the-practice of the artifact description and evaluation does not answer major questions of reproducibility.
In particular in terms of reproducibility of the experiments' software environment.
...
Badges have be shown to be an effective strategy incentisize authors to make the data of their works available \cite{kidwell2016badges, rowhani2017incentives}.
...
\begin{itemize}
  ...
  \item Wheter the paper had an \ad\ section
...
For isntance, authors could decide to delete or rename their repository, invalidating the URL given in the paper.
...
Platforms such as \grid\ \cite{grid5000} and Chamelon\ \cite{chameleon} offer such tools to their users (respectively \texttt{tgz-g5k}\ \cite{tgz-g5k} and \texttt{cc-snapshot}\ \cite{cc-snapshot}).
...
For instance, the lifetime of the \texttt{nvidia/cuda} Docker image is only 6 months, after 6 months, the Nvidia administrators of DockerHub delete the images\ \cite{nvidia_cuda_lifetime}.
...
The column \texttt{Loose image} on Figure \ref{fig:sw_envs} shows the papers that based their software environments on an image with a short \emph{lifetime}.
...
There are two main ways that the authors describe the workflow: fragile \texttt{bash} scripts or a \texttt{README} file that require to copy-paste the commands.
Some commands are sometime directly included in the paper, which makes it even harder to read and to copy-paste.
...
This study could be performed every year to assess the evolution through time of the community in terms of \emph{lifetime} of artifacts.
