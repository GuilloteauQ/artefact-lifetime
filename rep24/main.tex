...
In 2015, Collberg et al.\ \cite{collberg_repeatability_2015} studied the \repro\ of 402 experimental papers published in \emph{system} conferences and journals of 2011 and 2012.
...
The second most popular answer is quite interesting as \aeval\ processes were not very popular at the time of the survey (the very first one was in 2011 at the ESC/FSE conference).
...
It would be interesting to conduct again the survey today to see the impact on the \aeval\ on this question, as we believe that reproducibility and its challenges have now much more visibility in terms of citations.
...
This \adae\ process usually consists of the authors writing an \ad\ as an appendix of the paper to show how to get and use the artifact, how to install the dependencies, what are the different experiments and their estimated duration, etc. \cite{ae_tip, creating_successful_artifacts, ae_guidelines}.
...
Table \ref{tab:table:paper_confs} summarizes our evaluations, including the number of papers for each conference and how many of them have an \ad.
...
\begin{itemize}
  ...
  \item How the source code was shared: \texttt{git} repository (\eg\ GitHub, GitLab), Zenodo, Software-Heritage, or combination of solutions
...
The data presented in this paper has been collected \emph{manually by a single researcher}.
...
\begin{figure}
  ...
  \caption{Methods used by the authors to share their artifacts (154 in total). The state-of-the-practice is dominated by \texttt{git} URLs, Zenodo archives, and a combination of the two (\texttt{git+zenodo}).}\label{fig:how_repo_shared}
\end{figure}
...
However, in Figure \ref{fig:cache_bin} we see that for a bit more than half of the \ad s using an image, we could not find the source recipe, which prevents the readers from inspecting or rebuilding the image.
...
For this, authors indicate in the \ad\ the process to download the dependency and how to build it.
...
A popular way to manage a software environment on HPC systems is through \emph{Modules} \cite{furlani1991modules, modules}.
...
One drawback is that loading and unloading modules has side effects on the state of the system, and might thus not set back the system in its initial state.
...
However, a drawback of using Spack on an HPC system is that this directory consumes a lot of storage quota and inodes.
...
Artifacts with long \emph{longevity} are much more valuable and impactful as they allow for future researchers to validate the results and build on former research.
They deserve to be rewarded and have more visibility.
Table\ \ref{tab:longevity} proposes a grading framework to evaluate the \emph{longevity} of an artifact.
The artifact is evaluated on three \emph{longevity} criteria: how the source code is shared, the availability of the experimental setup, and the \emph{longevity} of the software environment itself.
We propose to grade each aspect on 5 levels going from insufficient (0/4) to best (4/4).
...
Figure \ref{fig:longevity_score} shows the distribution of scores for each of the three criteria (top three plots), and the global longevity score (bottom plot) for the papers surveyed in Section\ \ref{sec:sop}.
With the recommended threshold score of 3 out of 4, about 1 percent of the survey papers would have got the badge.
We can see that the most penalizing criteria is the one about software environment, where the majority of the papers have zero out of four, as they do not describe their software environment accurately enough to be stable through time.
