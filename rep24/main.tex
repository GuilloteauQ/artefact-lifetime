...
%\title{Study of the State-of-the-practice for sharing artifacts in top system conferences from the point-of-view of artifacts longevitys}
...
\affiliation{%
  \institution{IRIT, Université de Toulouse}
  \city{Toulouse}
  \country{France}
}
...
\begin{abstract}
  ...
  We argue that the notion of 'reproducibility' considered by such badges is limited, and misses important aspects of the reproducibility problem \fmc{or crisis?}.
  In this article, we survey 296 articles from 5 top conferences in (distributed) systems held in 2023.
  ...
  We also found that the state of practice does not address reproducibility in terms of artifact \emph{longevity} and expose 7 takeways that support this finding.
  To address this issue, we propose a new badge based on source code, experimental setup, and software environment, which will allow artifacts to withstand the test of time and support reproducibility in the long term.  
...
These defintions themselves rely on the International vocabulary of metrology\ \cite{defs_vim}.
...
"All these" raise the question of: What will happen when someone tries to rebuild the artifacts' environment 1, 5, or 10 years into the future?
The objective of science is to be based on robust work to advance the frontiers of knowledge (\emph{Stand on the shoulders of giants}\fmc{Need to find original citation for this}).
Such '\textbf{short term reproducibility}' is a major obstacle to scientific progress and in complete opposition to the science philosophy \fmc{Which is? We need to cite this or define this better.}.
...
However, even if variation is the end goal, we claim that the current state-of-the-practice of artifact description and evaluation does not yet answer the main questions of reproducibility \fmc{which are? or where have they been defined?. we need to refer to those here; NO, we want to fulfil the reproducibility properties... }, in particular in terms of the reproducibility of the experimental software environment.

This work analyzes the longevity of existing \ad s from the surveyed articles .... and exposes 7 takeaways... \fmc{TO COMPLETE THE SENTENCE}. 
...
In Section \ref{sec:sop}, we survey the articles from five top parallel and distributed systems conferences of 2023, and discuss the state-of-the-practice of artifact sharing.
...
Attempts have been made to define a "quality indicator" of research artifacts\ \cite{castell2024towards}.
\todo{more}.
...
The review process includes a thorough inspection of a submission's source code, the documentation of the software, and a run-through of some examples.
...
Table \ref{tab:table:article_confs} summarizes our evaluations, including the number of articles for each conference and how many of them have an \ad.
...
For example, here is the Software-Heritage archive of the repository of this article: \cite{artefact-lifetime}.
...
Similarly, some articles (in particular for the SuperComputing conference (\texttt{sc23})) used supercomputers to conduct experiments.
...
The column \texttt{Loose image} in Figure \ref{fig:sw_envs} shows the articles that based their software environments on an image with short \emph{longevity}.
...
Modules allow users to change their environment by "loading" and "unloading" packages and allow one to simply manage different versions of applications. 
...
Moreover, as the \texttt{modulefiles} are managed by the administrators, they do not have infinite longevity and might be unavailable in the future.
%\todo{Easybuild?}
...
The definition of packages through functions also faciliates their sharing and distribution.
...
The artifact is evaluated on three \emph{longevity} criteria: sharing of the source code, experimental setup used, and software environment.
...
The specifics of the grade for each criteria should be further discussed in the community, to reach a consensus on desired good practices.
...
We can see that the most penalizing criteria is the one about software environment, where the majority of the articles have zero out of four, as they do not describe their software environment accurately enough to be stable through time.
...
In Section \ref{sec:sop} of this article, we surveyed 296 articles from 5 top conferences in system and distributed systems of 2023.
For each article of these conferences, we gathered information about its artifacts and badges awarded.
